...
export class SlashtagsAuthServer extends Base {
   ...
   * @param {{onauth?: OnAuth, onmagiclink?: OnMagicLink}} [opts]
   */
  constructor (slashtag, opts) {
    super(slashtag)
    this.onauth = opts?.onauth || noauthz
  ...
  async auth (req, socket) {
    return await this.onauth(req, socket.remoteSlashtag)
  }
...
function noauthz () {
  throw new Error('Authz link not implemented')
}

/**
 * @typedef {import('@synonymdev/slashtag').SecretStream['remoteSlashtag']} RemoteSlashtag
 * @typedef {(token: string, remote: RemoteSlashtag) => boolean | Promise<boolean>} OnAuth
